...
const translationCodes = contentFileNames.filter(elem => elem !== 'content.md').map(elem => elem.replace(/^content\./, '').replace(/\.md$/, ''))
...
const generateContentAssets = () => {
    contentFileNames.forEach(fileName => {
        ...
        const content = fileName === 'content.md' ? {...infoAndChapters, translations: translationCodes } : infoAndChapters
    ...
    export default `
        ...
        writeFileSyncRecursive(filePath, beforeContent + JSON.stringify(content), 'utf-8')
...
const generateTranslationsMap = () => {
    ...
    const trString = (trCode) => `\t'${trCode}': require('./content.${trCode}.js'),\n` // "ru": require('./content.ru.js'),
    const translationsAsset = translationCodes.reduce((prevItem, currentItem) => prevItem + trString(currentItem), '')
    const translationsAssetFile = `//autogenerated from files: /${contentFolder}/content.LANG_CODE.md
// via /src/utils/createContent.js
export default {
${translationsAsset}}`

    writeFileSyncRecursive(path.join('./assets', 'translations.js'), translationsAssetFile, 'utf-8')
}
